            ...
            if($_POST){
                //if($_SESSION['permisosMod']['Permiso_Delete']){
                    ...
                    if($requestDelete == 'ok')
                    {
                        ...
                        $descripcionBT = 'Se elimino el proveedor ';//descripcion de lo que se hizo
            
                        $objetoBT = 36; //le manda el valor de 1 que significa que esta en el objeto de login, eso varia depende donde se encuentre el usuario
            ...
            if($request_rol > 0 )
            {
                if($option == 1)
                {
                    ...
                    //bitacora este codigo se pondra en cada uno de las acciones si se agrego o si actualizo o si se elimmino
						$fecha_actual = (date("Y-m-d"));
						$UsuarioBt = $_SESSION['userData']['id_usuario'];  //aqui es el usuario que hizo el cambio
						$eventoBT = "Agregó proveedor"; // evento de si se ingreso, actualizo o elimino 
						$descripcionBT = 'Se agregó un nuevo proveedor ';//descripcion de lo que se hizo
					...
					$fecha_actual = (date("Y-m-d"));
					$UsuarioBt = $_SESSION['userData']['id_usuario'];  //aqui es el usuario que hizo el cambio
					$eventoBT = "Actualizó proveedor"; // evento de si se ingreso, actualizo o elimino 
					$descripcionBT = 'Se actualizó un proveedor ';//descripcion de lo que se hizo
